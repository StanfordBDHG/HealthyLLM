...
class VitalSignsHandler: ToolHandler {
    ...
    private func getVitalSignAverage(for type: HKQuantityTypeIdentifier, weeksToAverage: Int = 4) async throws -> String {
        ...
        case .heartRate, .restingHeartRate, .respiratoryRate, .oxygenSaturation:
            ...
            let avgData = try await HealthDataFetcher.shared.getAverageQuantityValue(for: type, weeksToAverage: weeksToAverage)
            if var value = avgData.0 {
                ...
                let avgYearData = try await HealthDataFetcher.shared.getAverageQuantityValue(for: type, weeksToAverage: weeksPerYear)
        ...
        case .bloodPressureSystolic, .bloodPressureDiastolic:
            ...
            if let sys = systolicAvg.0, let dia = diastolicAvg.0 {
                ...
                let systolicYear = try await HealthDataFetcher.shared.getAverageQuantityValue(for: .bloodPressureSystolic, weeksToAverage: weeksPerYear)
                let diastolicYear = try await HealthDataFetcher.shared.getAverageQuantityValue(for: .bloodPressureDiastolic, weeksToAverage: weeksPerYear)
